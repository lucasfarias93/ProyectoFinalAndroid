...
public class ChangePasswordAsynctask extends AsyncTask<ConnectionParams, Void, List<String>> {
    Context context;
    Callback callback;
    ProgressDialog dialog;

    public ChangePasswordAsynctask(Context context, Callback callback, ProgressDialog dialog) {
        this.callback = callback;
        this.context = context;
    }

    public interface Callback {
        void changePassword(Boolean success);
    }
    ...
    protected void onPostExecute(List<String> result) {
        ...
        switch (searchType) {
            case 11:
                callback.changePassword(Boolean.valueOf(result.get(0)));
                break;
